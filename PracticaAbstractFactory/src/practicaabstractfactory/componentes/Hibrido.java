/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Interface.java to edit this template
 */
package practicaabstractfactory.componentes;

/**
 *
 * @author Antonio
 */
public abstract class Hibrido {
    protected String modelo, marca;
    protected int cv, consumo, autonomiaElectrica, plazas;
    protected float precio;

    public Hibrido() {
    }

    public Hibrido(String marca, String modelo, int cv, int consumo, int autonomiaElectrica,  int plazas, float precio) {
        this.marca = marca;
        this.modelo = modelo;
        this.cv = cv;
        this.consumo = consumo;
        this.autonomiaElectrica = autonomiaElectrica;
        this.precio = precio;
        this.plazas = plazas;
    }

    public String getModelo() {
        return modelo;
    }

    public void setModelo(String modelo) {
        this.modelo = modelo;
    }

    public int getCv() {
        return cv;
    }

    public void setCv(int cv) {
        this.cv = cv;
    }

    public int getConsumo() {
        return consumo;
    }

    public void setConsumo(int consumo) {
        this.consumo = consumo;
    }

    public int getAutonomiaElectrica() {
        return autonomiaElectrica;
    }

    public void setAutonomiaElectrica(int autonomiaElectrica) {
        this.autonomiaElectrica = autonomiaElectrica;
    }

    public float getPrecio() {
        return precio;
    }

    public void setPrecio(float precio) {
        this.precio = precio;
    }

    public int getPlazas() {
        return plazas;
    }

    public void setPlazas(int plazas) {
        this.plazas = plazas;
    }

    public String getMarca() {
        return marca;
    }

    public void setMarca(String marca) {
        this.marca = marca;
    }
    
    public abstract void mostrarCaracteristicas();
}
